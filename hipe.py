...
def hierarchical_perturbation(model,
    ...
    with torch.no_grad():
        ...
        depth_sal = torch.zeros((1, 1, input_y_dim, input_x_dim), device=dev)
        ...
        while (num_cells*2) <= (dim/4):
            ...
            while len(masks_list) > 0:
                ...
                depth_sal = torch.sum(sal, dim=(0, 1))
        ...
        if resize is not None:
            ...
            depth_sal = F.interpolate(depth_sal, (resize[1], resize[0]), mode=interp_mode)
        if return_info:
            return saliency, {'thresholds': thresholds_d_list, 'masks': masks_d_list, 'total_masks': total_masks}
        else:
            return saliency, depth_sal, total_masks
